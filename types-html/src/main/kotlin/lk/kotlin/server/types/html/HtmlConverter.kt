...
class HtmlConverter : Parser, Renderer {
    ...
    fun <T> HtmlSubConverter<T>.renderFormSafe(info: LevelInfo<T>, to: Appendable) = try {
        renderForm(info, to)
    } catch (e: Exception) {
        e.printStackTrace()
    }

    fun <T> HtmlSubConverter<T>.parseSafe(info: LevelInfo<T>): T? = try {
        parse(info)
    } catch (e: Exception) {
        e.printStackTrace()
        null
    ...
    fun <T> HtmlSubConverter<T>.renderParametersSafe(info: LevelInfo<T>, to: MutableMap<String, String>) = try {
